...
var tokensAverage = normalizedCandidates.Count > 0
    ? tokensTotal / (double)normalizedCandidates.Count
    : 0;
var tokensAverageRounded = Math.Round(tokensAverage, 1);
var tokensAverageText = tokensAverageRounded % 1 == 0
    ? tokensAverageRounded.ToString("0")
    : tokensAverageRounded.ToString("0.0");

Console.WriteLine($"normalized_total={normalizedCandidates.Count}");
Console.WriteLine($"tokens_total={tokensTotal}");
Console.WriteLine($"tokens_avg={tokensAverageText}");
